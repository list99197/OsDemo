...
public class DeviceController {
   ...
   public DeviceController() {
		...
		getPid(useTable,waitTable);
	}
	public void getPid(PCB[][] useTable,PCB[][] waitTable) {
		for(int i=0;i<2;i++) {
			if(useTable[0][i]!=null) {
				this.deviceList.get(i).setRunPIDBy(useTable[0][i].toString());
			}else {
				this.deviceList.get(i).setRunPIDBy("");
			}
		}
		for(int i=0;i<3;i++) {
			int j;
			...
			if(useTable[1][i]!=null) {
				...
				this.deviceList.get(j).setRunPIDBy("");
			}
		}
		for(int i=0;i<3;i++) {
			int j;
		...
		for(int i=0;i<waitTable[0].length;i++) {
			...
			this.deviceList.get(0).setWaitPIDBy("");
			this.deviceList.get(1).setWaitPIDBy("");
		}
		String string2 = "";
		...
		for(int i=0;i<waitTable[1].length;i++) {
			...
			this.deviceList.get(2).setWaitPIDBy("");
			this.deviceList.get(3).setWaitPIDBy("");
		...
		for(int i=0;i<waitTable[2].length;i++) {
			...
			this.deviceList.get(4).setWaitPIDBy("");
			this.deviceList.get(5).setWaitPIDBy("");
			this.deviceList.get(6).setWaitPIDBy("");
		}
	}
